...
if (len(sys.argv) != 3):
    print("How to execute the code: \n python3 qlearning.py N t\n\n where N is the destination and t the instant")
    exit()
...
if (int(sys.argv[2]) < 0 or int(sys.argv[2]) >= 10000):
    print("t has to be between 0 and 9999")
    exit()

print("Compute of Q for sending to {} at {}".format(sys.argv[1], sys.argv[2]))

nSat = 100
Q = np.zeros([nSat, nSat])
out = "q/to_{}/q_instant_{}.csv".format(int(sys.argv[1]), int(sys.argv[2]))
...
swarmNetwork = SwarmNetwork(end, int(sys.argv[2]))
...
for i in range(num_episodes):
    ...
    while True:
        ...
        couple = [a, distance]
        Q[s, a] = Q[s, a] + lr*(reward + gamma * np.max(Q[s1,:]) - Q[s, a]) # Fonction de mise à jour de la Q-table
        cumul_reward += reward
        s = s1
...
print("Compute of Q for sending to {} at {}\n".format(sys.argv[1], sys.argv[2]))
print("Number of Success: {}/{}".format(swarmNetwork.getSucess(), num_episodes - 100))
